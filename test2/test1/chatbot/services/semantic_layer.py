...
def get_sample_data(tables: list, sample_size: int = 3) -> str:
    """
    Получить примеры данных из указанных таблиц.
    Возвращает formatted строку с первыми N строками каждой таблицы.
    """
    if not tables:
        return ""
    ...
    try:
        ...
        for table in sorted(tables):
            try:
                ...
                # Форматируем
                lines = [f"{table} ({len(col_names)} колонок):"]
                lines.append("  | " + " | ".join(col_names[:8]) + (" | ..." if len(col_names) > 8 else "") + " |")
                lines.append("  |" + "|".join(["-" * (len(c) + 2) for c in col_names[:8]]) + ("|" + "-" * 5 if len(col_names) > 8 else "") + "|")

                for row in rows:
                    vals = [str(v)[:30] if v is not None else "NULL" for v in row[:8]]
                    lines.append("  | " + " | ".join(vals) + (" | ..." if len(row) > 8 else "") + " |")
